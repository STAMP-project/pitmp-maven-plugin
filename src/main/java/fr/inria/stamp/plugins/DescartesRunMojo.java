...
public class DescartesRunMojo extends AbstractMojo
{
   ...
   // **********************************************************************
   // ******** methods
   public void initializeDescartes()
   {
      System.out.println("################ DescartesRunMojo.initializeDescartes");
      if (goalStrategy.rootProject == null)
      {
         goalStrategy.rootProject = getProject();
      }
      System.out.println("# rootProject: " + goalStrategy.rootProject.getArtifactId());
   }
   ...
   protected Option<CombinedStatistics> analyse() throws MojoExecutionException
   {
      ...
      System.out.println("################ DescartesRunMojo.analyse");

      goalStrategy.projectList.add(getProject());
      goalStrategy.projectData.add(new Mojo2Options(this,
        new SurefireConfigConverter(), filter).convert());
      goalStrategy.currentProject = goalStrategy.projectList.size() - 1;

      // data.setFailWhenNoMutations(false);

      result = Option.some(goalStrategy.execute(detectBaseDir(),
         goalStrategy.projectData.get(goalStrategy.currentProject),
  ...
  protected final RunDescartesStrategy        goalStrategy;

  public DescartesRunMojo() {
    // this(new RunPitStrategy(), new DependencyFilter(new PluginServices(
    this(new RunDescartesStrategy(), new DependencyFilter(new PluginServices(
        DescartesRunMojo.class.getClassLoader())), new PluginServices(
        DescartesRunMojo.class.getClassLoader()), new NonEmptyProjectCheck());
  }

  public DescartesRunMojo(final RunDescartesStrategy strategy, final Predicate<Artifact> filter,
